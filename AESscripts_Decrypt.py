...
root.maxsize(width=250, height=50)

loadButton = Button(root, text="Load File(s)", command=enc.load_file)
#encryptButton = Button(root, text="Encrypt File", command=lambda: [f() for f in [enc.encrypt_all_files]])
decryptButton = Button(root, text="Decrypt File", command=lambda: [f() for f in [enc.decrypt_all_files]])
